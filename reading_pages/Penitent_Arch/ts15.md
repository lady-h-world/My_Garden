...
Now let's add `m=7` in SARIMA while using `(p=10, d=0, q=0)` as an example. Note, using `p=10` is not conflict with what we just found `p=7` is ideal, this is just an example, check "Step 3" below for more.

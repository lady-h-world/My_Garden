...
* Configuration of `model_components`, `evaluation_period` and `metadata` can be the same as Silverkite, since they are not dependent on the algorithm of Silverkite or Prophet, but more dependent on Greykite framework. Also note, in Prophet template, we no longer need `custom` to specify which algorithms to use, since Prophet itself serves as the forecasting algorithm.
...
Looking at the R2 and MAPE backtest results, omparing with the best ARIMA results we got, Silverkite template improved MAPE from 1.14 to 0.77 (32% decrease) and improved R2 from 0.49 to 0.77 (57% increase).
...
Although using Prophet template, we got better model performance in both MAPE and R2, its model summary is a shame. Look, this is all what you can get:

<p align="left">
<img src="https://github.com/lady-h-world/My_Garden/blob/main/images/Garden_Totem_images/forecasting/prophet_model_summary.png" width="1442" height="406" />
</p>

...
When exploring the relationship between variables in a multivariate time series, we also want to know whether one variable can forecast another variable. Granger causality is used for this type of analysis.
...
Before applying granger causality, suggest to do some data preprocessing:
* Remove highly correlated variables, to reduce unnecessary calculation. Because if a variable can forecast another variable or can be forecasted through another variable, then its highly correlated variable can have similar effect.
* Make the multivariate time series stationary. This was already done in previous stationary analysis step.
...
After data preprocessing, now let's look at the core logic of granger causality analysis.
...
When we are using `grangercausalitytests(df[[col1, col2]])`, the null hypothesis and alternative hypothesis:
* H0: col2 does not granger cause col1
* H1: col2 granger cause col1

Therefore when we have found col2 granger causes col1 but col1 doesn't granger cause col2, then we will get output showing "col2 causes col1", and vice versa.
...
Besides checking exploring assumed causality, we can also use the output to understand the interchangable influence between variables in a multivariate time series. If we found such influence exists, then models like VAR can be a good choice to be applied to the data for forecasting, outlier detection, etc.

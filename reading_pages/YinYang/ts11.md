...
Before applying VAR, we need to convert all the input variables to stationary, because this is the premise of VAR. In fact we have already done this in the data exploration section, after doing 1st order differencing on Humidity and HumidityRatio, we can get each time series as both differencing stationary and trending stationary.
...
Next, we will find the optimal order for VAR.

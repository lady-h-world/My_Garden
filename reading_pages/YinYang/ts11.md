...
Have to admit, Greykite doesn't have a mature outlier detection method. It only has `ZscoreOutlierTransformer()` function which will replace its detected outliers to NAN directly, which can be risky, since some outliers will benefit later model forecasting and better not to be removed. Moreover, it only applies to univariate time series.
...
2. If this z-score value larger than a cutoff threshold, then the observation will be considered as an outlier, and Greykite will repalce it to NAN

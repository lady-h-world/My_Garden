...
Remember that in Kats changepoint detection, Lady H. simply could not make it works on the sales data? With Greykite, the changepoint detection works on all the 3 time series (sales, temperature and humidity).

#### Trend Changepoint Detection

The configuration for trend detection is shown as below, Greukite's methods is often flexible for both univariate time series and multivariate time serie.
...
These are the trend changepoints for temperature and humidity data:
...
The configuration for seasonality changepoint detection is similar but just to change to another function:
...
greykite will show seasonality changepoints for different components (daily, weekly, yearly). For example, for the sales data, it detected weekly and yearly changepoints:
...
These are the seasonality changepoints for temperature and humidity data:

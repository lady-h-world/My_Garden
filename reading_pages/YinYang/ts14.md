...
Let's manually assign ARIMA parameters first. Based on the thumb rules above, we should at least use `p`. Meanwhile, when checking PACF, there are several lags showing significant autocorrelation. Lady H. tried `p` between 7 and 10 and found `p=10` got better AIC, BIC performance. The detail is shown as below:
...
The comparision between forecatsed values and the real testing data is shown below, there is not much overlap between the green curve and the orange curve. We are using both MAPE (mean absolute percentage error) and R2 as the evaluation metrics.
...
How can we improve the forecasting performance further? As we can see, applying the thumb rules can be tricky since the reality of the dataset is often more complex than expected. Often times, it's hard to decide ARIMA parameter values by only looking at PCF and PACF. A better solution is to decide the parameter values using HPO (hyperparameter tuning).

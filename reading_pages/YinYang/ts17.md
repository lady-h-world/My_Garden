...
1. `ModelComponentsParam` is where you can config feature generation and model settings.
   ...
   2. `events` is where to specify special events features that be useful in forecasting. `holidays_to_model_separately` is to tell Silverkite to use specified holidays independently, and the rest of the holidays will be used together as 1 event.
